...
const router = Router();
const coll = () => db.collection('books'); // reference to 'books' collection

// ----------------------------
// Public endpoints
// ----------------------------

// Public list/search endpoint
router.get('/', async (req, res, next) => {
  try {
    const q = (req.query.q || '').toLowerCase(); // optional search query
    const snap = await coll().get();
    let rows = snap.docs.map(d => ({ id: d.id, ...d.data() }));

    if (q) {
      // Filter by title, author, or tags if query exists
